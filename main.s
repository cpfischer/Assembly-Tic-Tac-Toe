...
#------------------------------------------------------------------------------
# Checks if X is about to win to block it
#------------------------------------------------------------------------------												
blockX:			li		$t0, 0					# i
				li		$t1, 1					# X value

blockLoop:		bge		$t0, $s3, firstOpen 	# Searches for a way to block X
				
				add		$t4, $s0, $t0			# t4 = &board[i]
				lw		$t5, 0($t4)				# t5 = board[i]
				bne		$t5, $zero, blockElse	# checks if spot is empty
				sw		$t1, 0($t4)				# sets &board[i] = 2 = 'X'
				addiu	$sp, $sp, -16			# moves sp down 16
				...
				li		$t6, 1
				beq		$v0, $t6, exitBlock	# if X win, exitBlock
				sw		$zero, 0($t4)		# else, board[i] = 0 = ' '
				
blockElse:		addi	$t0, 4		# i + 4
				j		blockLoop
				
exitBlock:		li		$t1, 2				# O value
				sw		$t1, 0($t4)			# stores O in board[i]
				j		exitBot
								
#------------------------------------------------------------------------------
# checks for the first open space for O
#------------------------------------------------------------------------------				
